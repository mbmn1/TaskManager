...
import { Plus, Users, FolderKanban, ShieldCheck, UserCheck, AlertCircle, Sparkles, Trash2, Mail, Phone, Pencil, X, Check, Building2, History, Search, ChevronDown, ChevronUp } from "lucide-react";
import { Employee, Project, AuditLog, Task } from "../types";
import { addEmployee, createProject, deleteEmployee, deleteProject, updateEmployee, updateProject, subscribeAuditLogs, deleteCompletedTasks, fetchAllTasks, fetchEmployees, fetchProjects } from "../lib/dbService";
...
const SECRET_KEY = "Mbmn@B!#!951";



export default function AdminPanel({ currentUser, employees, projects, allTasks, mode, onDataUpdate }: AdminPanelProps) {
  ...
  const otherEmployees = employees.filter(e => e.role === "employee");
  ...
  const handleDeleteCompletedTasks = async (projectId: string, projectName: string) => {
    ...
    if (window.confirm(`Are you sure you want to permanently delete all completed and rejected tasks for "${projectName}"?`)) {
      try {
        setDeleteError(null);
        ...
        if (res.error) {
          ...
          onDataUpdate();
          alert(`Successfully cleaned ${res.deletedCount || 0} tasks from "${projectName}".`);
        }
      } catch (err: any) {
        setDeleteError(err.message || "Failed to clean up tasks.");
      }
    }
  };
                    ...
                    {employees.filter(emp => {
                      ...
                      return (
                        <tr key={emp.email || emp.phone} className="hover:bg-slate-50/40 transition-colors">
                          <td className="p-3">
                            <div className="flex items-center gap-3">
                              <div className="w-8 h-8 rounded-full bg-slate-100 text-slate-600 font-bold text-[11px] grid place-content-center border border-slate-200">
                                {initials}
                              </div>
                              <div>
                          ...
                          return (
                            <div
                              ...
                              className={`flex items-center justify-between p-2.5 cursor-pointer rounded-lg transition-all ${
                                isSelected
                                  ? 'bg-indigo-50/60 text-indigo-950 font-bold border border-indigo-100'
                            ...
                            >
                              <div className="overflow-hidden mr-2">
                                <div className="text-[11px] font-bold truncate">
                                  {emp.name}
                                  {emp.role === 'client' && <span className="text-amber-600 font-normal"> (Client)</span>}
                                </div>
              ...
              {projects.length === 0 ? (
                ...
                <div className="divide-y divide-slate-100">
                  {projects.map((proj) => {
                    ...
                    const cleanableTasksCount = allTasks.filter(t => t.projectId === proj.id && (t.status === 'completed' || t.status === 'rejected')).length;
                    return (
                      <div key={proj.id} className="p-5 hover:bg-slate-50/40 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="space-y-1 max-w-lg overflow-hidden">
                          ...
                          {proj.description && (
                            <>
                              <p className={`text-xs text-slate-500 leading-relaxed max-w-md whitespace-pre-wrap ${!isDescExpanded && 'line-clamp-2'}`}>
                                {proj.description}
                              </p>
                              {proj.description.length > 150 && (
                                <button onClick={() => setIsDescExpanded(!isDescExpanded)} className="text-[10px] font-bold text-indigo-600 hover:underline mt-1 flex items-center gap-1">
                                  {isDescExpanded ? 'Read Less' : 'Read More'} {isDescExpanded ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                                </button>
                              )}
                            </>
                          )}
                        ...
                        <div className="flex flex-wrap items-center gap-2 self-end sm:self-center shrink-0">
                          {cleanableTasksCount > 0 ? (
                            <button
                              ...
                              title="Delete all completed and rejected tasks for this project"
                            >
                              <Trash2 className="w-3.5 h-3.5 text-amber-600" />
                              Clean Tasks ({cleanableTasksCount})
                            </button>
                          ) : (
                            <button
                              disabled
                        ...
                        {employees.filter(e => e.role !== 'admin').map((emp) => {
                            ...
                            return (
                              <div
                                ...
                                isSelected
                                  ? 'bg-indigo-50 text-indigo-900 font-bold border border-indigo-100/50'
                                  : 'hover:bg-slate-100 text-slate-600'
                                }`}
                              >
                                <div className="overflow-hidden mr-2">
